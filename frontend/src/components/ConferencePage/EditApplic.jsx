...
const EditApplic = ({application, isOpen, setIsOpen, changeData, conference}) => {
    ...
    const onOk = () => {
        changeData({...applicData, updated_at: new Date()});
        setIsOpen(false);
    }

    const onCancel = () => setIsOpen(false);

    const addCoauthorCount = () => setApplicData({...applicData, coauthors: [...applicData.coauthors, {name:"", surname:"", patronymic:"", email:""}]});

    const deleteCoauthor = () => setApplicData({...applicData, coauthors: [...(applicData.coauthors.slice(0, applicData.coauthors.length-1))]})
